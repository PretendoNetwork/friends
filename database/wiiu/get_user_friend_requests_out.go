...
func GetUserFriendRequestsOut(pid uint32) (*types.List[*friends_wiiu_types.FriendRequest], error) {
	...
	rows, err := database.Manager.Query(`SELECT id, recipient_pid, sent_on, expires_on, message, received FROM wiiu.friend_requests WHERE sender_pid=$1 AND accepted=false`, pid)
	if err != nil {
	...
	for rows.Next() {
		...
		userInfo, err := GetUserPrincipalBasicInfo(recipientPID)
		if err != nil {
			return nil, err
		}
